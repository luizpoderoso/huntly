<script lang="ts">
    ...
    const triggerContent = $derived(
        interviewTypes.find(t => t === selectedType) ?? 'Select type'
    );
        ...
        <form onsubmit={handleSubmit} class="flex flex-col gap-4">
            <div class="flex flex-col gap-2">
                ...
                {#if fieldError('interviewType')}
                    <p class="text-destructive text-xs">{fieldError('interviewType')}</p>
                {/if}
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                {#if fieldError('scheduledAt')}
                    <p class="text-destructive text-xs">{fieldError('scheduledAt')}</p>
                {/if}
            </div>
            ...
            <Dialog.Footer>
                ...
                <Button type="submit" disabled={!scheduledAt}>
                    Add interview
                </Button>
            </Dialog.Footer>
        </form>
