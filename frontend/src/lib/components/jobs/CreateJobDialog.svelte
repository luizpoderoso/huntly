	...
	<Dialog.Content class="sm:max-w-md">
		...
		<form onsubmit={handleSubmit} class="flex flex-col gap-4">
			<div class="flex flex-col gap-2">
				...
				<Input id="companyName" bind:value={companyName} placeholder="Acme Corp" />
				{#if fieldError('companyName')}
					<p class="text-xs text-destructive">{fieldError('companyName')}</p>
				{/if}
			</div>

			<div class="flex flex-col gap-2">
				<Label for="position">Position *</Label>
				<Input id="position" bind:value={position} placeholder="Software Engineer" />
				{#if fieldError('position')}
					<p class="text-xs text-destructive">{fieldError('position')}</p>
				{/if}
			</div>

			<div class="flex flex-col gap-2">
				<Label for="jobUrl">Job URL</Label>
				<Input id="jobUrl" bind:value={jobUrl} placeholder="https://..." />
				{#if fieldError('jobUrl')}
					<p class="text-xs text-destructive">{fieldError('jobUrl')}</p>
				{/if}
			</div>
			...
			<div class="grid grid-cols-3 gap-2">
				<div class="flex flex-col gap-2">
					...
					<Input id="salaryMin" type="number" bind:value={salaryMin} placeholder="50000" />
				</div>
				<div class="flex flex-col gap-2">
					<Label for="salaryMax">Max salary</Label>
					<Input id="salaryMax" type="number" bind:value={salaryMax} placeholder="80000" />
				</div>
				<div class="flex flex-col gap-2">
					<Label for="salaryCurrency">Currency</Label>
					<Input id="salaryCurrency" bind:value={salaryCurrency} placeholder="USD" />
				</div>
			</div>
			...
			<Dialog.Footer>
				...
				<Button class="hover:cursor-pointer" type="submit" disabled={jobsStore.loading}>
