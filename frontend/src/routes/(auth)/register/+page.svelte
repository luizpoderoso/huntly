...
<Card.Root>
	...
	<Card.Content>
		<form onsubmit={handleSubmit} class="flex flex-col gap-4">
			{#if errors?.general}
				<p class="text-center text-sm text-destructive">{errors.general[0]}</p>
			{/if}
			<div class="flex flex-col gap-2">
				<Label for="fullName">Full name</Label>
				<Input id="fullName" type="text" bind:value={fullName} placeholder="John Doe" />
				{#if fieldError('fullName')}
					<p class="text-xs text-destructive">{fieldError('fullName')}</p>
				{/if}
			</div>

			<div class="flex flex-col gap-2">
				<Label for="username">Username</Label>
				<Input id="username" type="text" bind:value={username} placeholder="johndoe" />
				{#if fieldError('username')}
					<p class="text-xs text-destructive">{fieldError('username')}</p>
				{/if}
			</div>
			...
			<div class="flex flex-col gap-2">
				...
				{#if fieldError('password')}
					<p class="text-xs text-destructive">{fieldError('password')}</p>
				{/if}
			</div>
